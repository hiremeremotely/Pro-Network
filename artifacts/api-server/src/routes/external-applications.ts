...
async function exchangeOAuthCode(
  ...
  const data = (await r.json()) as { refresh_token?: string; access_token?: string; error?: string };
  if (!r.ok || data.error) throw new Error(`Token exchange failed: ${data.error ?? r.status}`);
  return data.refresh_token ?? data.access_token ?? "";
}

async function fetchGmailInbox(accessToken: string): Promise<InboxEmail[]> {
...
async function fetchOutlookInbox(accessToken: string): Promise<InboxEmail[]> {
  const filter = encodeURIComponent(
    "contains(subject,'application') or contains(subject,'interview') or contains(subject,'offer')",
  );
  const url =
    `https://graph.microsoft.com/v1.0/me/messages` +
    `?$filter=${filter}&$select=id,subject,from,receivedDateTime,bodyPreview&$top=50` +
...
router.get("/email-integration/callback", async (req, res): Promise<void> => {
  ...
  if (!IS_DEMO && code) {
    try {
      rawToken = await exchangeOAuthCode(provider, code, redirectUri);
    } catch {
      rawToken = `oauth_code_exchanged:${code.slice(0, 8)}`;
    }
  } else {
